import { type JsonData, useDeepEffect } from '@beesoft/common';
import cx from 'classnames';
import { type ReactNode, useCallback, useEffect, useId, useRef, useState } from 'react';
import { ItemScrollerPage } from './item-scroller-page.component.tsx';
import type { ItemScrollerProps } from './item-scroller.props.ts';
...
const ItemScroller = ({
  ...
  const nextPageIndex = useRef<number>(undefined);
  ...
  const resizeCallback = useCallback((entries: Array<ResizeObserverEntry>) => {
    for (let i = 0, length = entries.length; i < length; i++) {
      ...
      const pageHeight = entries[i]?.contentRect.height || 0;
  ...
  const scrollerStyle = cx('bsh:overflow-y-auto', className);
