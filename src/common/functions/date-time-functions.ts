...
export function getMonthMatrix(matrixDate: Date, locale: Locale, loadOtherMonths = true) {
  ...
  for (let row = 0, length = rowCount; row < length; row++) {
    for (let col = row > 0 ? 0 : firstDayOfMonthNumber; col < 7; col++) {
      ...
      if (currentMonthDate) {
        ...
        const matrixRowValue = monthMatrix[row];
        const matrixColumnValue = matrixRowValue?.[col];
        if (matrixColumnValue) {
          matrixColumnValue.dayValue = currentMonthDate;
        }
      }
  ...
  if (firstDayOfMonthNumber > 0 && loadOtherMonths) {
    for (let firstDay = 0; firstDay < firstDayOfMonthNumber; firstDay++) {
      const matrixRowValue = monthMatrix[0];
      const matrixColumnValue = matrixRowValue?.[firstDay];
      if (matrixColumnValue) {
        matrixColumnValue.dayValue = subDays(firstDayInMonth, firstDayOfMonthNumber - firstDay);
        matrixColumnValue.isCurrent = false;
      }
    }
  }
  ...
  if (lastDayOfMonthNumber > -1 && loadOtherMonths) {
    for (let lastDay = 6; lastDay > lastDayOfMonthNumber; lastDay--) {
      const matrixRowValue = monthMatrix[rowCount - 1];
      const matrixColumnValue = matrixRowValue?.[lastDay];
      if (matrixColumnValue) {
        matrixColumnValue.dayValue = addDays(lastDayInMonth, lastDay - lastDayOfMonthNumber);
        matrixColumnValue.isCurrent = false;
      }
    }
  }
...
export function getTranslatedMonthMatrix(locale: Locale) {
  ...
  for (let row = 0; row < 3; row++) {
    for (let column = 0; column < 4; column++) {
      const rowMonths = months[row];
      let columnMonths = rowMonths?.[column];
      if (columnMonths) {
        columnMonths = {
...
export function getTranslatedYearMatrix(matrixDate: Date, locale: Locale) {
  ...
  for (let row = 0; row < 3; row++) {
    for (let column = 0; column < 4; column++) {
      const matrixYear = matrixYears[yearCount++];
      const rowYears = years[row];
      let columnYears = rowYears?.[column];
      if (columnYears && matrixYear) {
        columnYears = matrixYear.toLocaleDateString(locale.code, {
          year: 'numeric',
        });
      }
...
export function parseDateRange(dateRangeValue: string, locale?: Locale) {
  const [dateToParse1, dateToParse2] = dateRangeValue.split('-');
  if (dateToParse1 === undefined || dateToParse2 === undefined) return undefined;

  const dateValue1 = parseDate(dateToParse1.trim(), locale);
  if (!dateValue1) return undefined;

  const dateValue2 = parseDate(dateToParse2.trim(), locale);
